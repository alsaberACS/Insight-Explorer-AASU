...
df1_dedup <- df1_dedup |>
  mutate(
    age_group = fct_collapse(
      age_group,
      "Elderly (60+)" = c("Elderly (60-74)", "Old (75-89)")
    )
  )
...
ggplot(prev, aes(x = prop, y = fct_rev(age_group))) +
  geom_vline(
    xintercept = mean(df1_dedup$target == "Disease"),
    linetype = "dashed", colour = "grey60"
  ) +
  geom_errorbar(aes(xmin = lower, xmax = upper), orientation = "y",
                width = 0, linewidth = 0.7, colour = "#2C5F8A") +
  geom_point(size = 4, colour = "#2C5F8A") +
  geom_text(aes(label = scales::percent(prop, accuracy = 1)),
            vjust = -1.2, size = 3.4, colour = "grey20") +
  geom_text(aes(label = paste0("n = ", n)), x = 0.02,
            hjust = 0, size = 3, colour = "grey45") +
  scale_x_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(
    x = "Heart disease prevalence (95% CI)",
    y = NULL,
    caption = "Dashed line: overall prevalence"
  ) +
...
# Step 20 faceted interaction/effect plot: predicted P(disease) vs age by chest pain
# Fit a model allowing the age effect to vary by chest pain type
hd_model_int <- glm(
  target ~ age * cp + chol,
  data = df1_dedup,
  family = binomial
...
# Prediction grid: age range by cp, cholesterol held at its mean
grid <- expand_grid(
  age = seq(min(df1_dedup$age), max(df1_dedup$age), by = 1),
  cp = factor(levels(df1_dedup$cp), levels = levels(df1_dedup$cp)),
  chol = mean(df1_dedup$chol)
)

pred <- predict(hd_model_int, newdata = grid, type = "link", se.fit = TRUE)
grid <- grid |>
  mutate(
    fit = plogis(pred$fit),
...
ggplot(grid, aes(x = age, y = fit)) +
  ...
  labs(
    ...
    caption = "Cholesterol held at sample mean; bands are 95% CIs"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    panel.grid.minor = element_blank(),
